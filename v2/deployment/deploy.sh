...
# BUILD POI
echo "Buiding API-POI image..."
echo "Changing directory to $GITOHTEAMDIRPATH/apis/poi/web..."
cd "$GITOHTEAMDIRPATH/apis/poi/web"
az acr build --image devopsoh/api-poi:latest --registry $ACRNAME --file Dockerfile .

# BUILD TRIPS
echo "Buiding API-TRIPS image..."
echo "Changing directory to $GITOHTEAMDIRPATH/apis/trips..."
cd "$GITOHTEAMDIRPATH/apis/trips"
az acr build --image devopsoh/api-trips:latest --registry $ACRNAME --file Dockerfile .

# BUILD USER-JAVA
echo "Buiding API-USER-JAVA image..."
echo "Changing directory to $GITOHTEAMDIRPATH/apis/user-java..."
cd "$GITOHTEAMDIRPATH/apis/user-java"
az acr build --image devopsoh/api-user-java:latest --registry $ACRNAME --file Dockerfile .

# BUILD USERPROFILE
echo "Buiding API-USERPROFILE image..."
echo "Changing directory to $GITOHTEAMDIRPATH/apis/userprofile..."
cd "$GITOHTEAMDIRPATH/apis/userprofile"
az acr build --image devopsoh/api-userprofile:latest --registry $ACRNAME --file Dockerfile .

# BUILD TripViewer
echo "Buiding Tripviewer image..."
...
az webapp create --resource-group $RGNAME --plan $APPSVCPLANNAME --name "openhack${RGSUFFIX}poi" --deployment-container-image-name "${ACRNAME}.azurecr.io/devopsoh/api-poi:latest"
...
az webapp create --resource-group $RGNAME --plan $APPSVCPLANNAME --name "openhack${RGSUFFIX}trips" --deployment-container-image-name "${ACRNAME}.azurecr.io/devopsoh/api-trips:latest"
...
az webapp create --resource-group $RGNAME --plan $APPSVCPLANNAME --name "openhack${RGSUFFIX}userjava" --deployment-container-image-name "${ACRNAME}.azurecr.io/devopsoh/api-user-java:latest"
...
az webapp create --resource-group $RGNAME --plan $APPSVCPLANNAME --name "openhack${RGSUFFIX}userprofile" --deployment-container-image-name "${ACRNAME}.azurecr.io/devopsoh/api-userprofile:latest"
